...
import silence.server as s_server #imported as absolute module to avoid circular import problems.
    ...
    for jsonfile in endpoint_paths_json:
        with open(jsonfile, "r") as ep:
            ...
            for endpoint in endpoints:
                ...
                s_server.endpoint.setup_endpoint(endpoint['route'], endpoint['method'], endpoint['sql'], **kwargs)
...
def load_default_endpoints():
    ...
    if settings.ENABLE_SUMMARY:
        s_server.manager.API_SUMMARY.register_endpoint({
                "route": route_prefix,
                "method": "GET",
                "desc": "Returns the data regarding the API endpoints",
        })
        s_server.manager.APP.add_url_rule(route_prefix, "APItreeHELP", show_api_endpoints, methods=["GET"])
    ...
    if settings.ENABLE_LOGIN:
        ...
        s_server.manager.APP.add_url_rule(login_route, "login", s_server.default_endpoints.login, methods=["POST"])
    ...
    if settings.ENABLE_REGISTER:
        ...
        s_server.manager.APP.add_url_rule(register_route, "register", s_server.default_endpoints.register, methods=["POST"])
...
def show_api_endpoints():
    return jsonify(s_server.manager.API_SUMMARY.get_endpoint_list()), 200